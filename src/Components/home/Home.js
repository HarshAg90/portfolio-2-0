...
export default function Home({themeFn}) {
  ...
  //const [hi, setHi] = useState([hellowords]);
  const count = 0;
  async function hello_animate(){
    count = count %6;
    document.querySelector("hello").classList.add("enter");
    document.querySelector("hello")
    await sleep(200);
    document.querySelector("hello").classList.remove("enter");
    await sleep(2000);
    document.querySelector("hello").classList.add("exit");
    await sleep(200);
    document.querySelector("hello").classList.remove("exit");
    ...
    <section id='home'>
        <div id="particles-js">
            <div className="info_co">
                {/* <h1 id='hello'>{hi}</h1> */}
                {/* <h1 id='hello_cl'>{hi}</h1> */}
                <h1 className="name">HARSH AGNIHOTRI</h1>
                <h2>Tech Finatic, And FullStack web and app developer</h2>
            </div>
